    ...
    if (pathname === '/marcar-como-eliminado' && req.method === 'PUT') {
        ...
        req.on('end', () => {
            ...
            connection.query('UPDATE empleado SET SINO = 0 WHERE ID = ?', [id], (error, results) => {
                if (error) {
                    console.error('Error al marcar como eliminado en la base de datos:', error);
                    res.writeHead(500);
                ...
                res.writeHead(200);
                res.end('Empleado marcado como eliminado correctamente en la base de datos');
            });
        });
        ...
        if (idEmpleado) {
            ...
            connection.query('SELECT ID, NOMBRE, APELLIDO, EMAIL, TELEFONO, IMG FROM empleado WHERE ID = ?', [idEmpleado], (error, results) => {
                if (error) {
                    console.error('Error al consultar la base de datos:', error);
                    res.writeHead(500);
                    res.end('Error al consultar la base de datos');
        ...
        req.on('end', () => {
            ...
            if (id) {
                ...
                connection.query('UPDATE empleado SET NOMBRE=?, APELLIDO=?, EMAIL=?, TELEFONO=? WHERE ID=?',
                    [nombre, apellido, email, telefono, id], (error, results) => {
                        if (error) {
